...
stdenv.mkDerivation {
  ...
  postInstall = ''
        ...
        if [ ! -e "$out/bin/wine" ]; then
          echo "Missing Wine wrapper: $out/bin/wine" >&2
          exit 1
        fi
        if [ ! -e "$out/bin/wineserver" ]; then
          echo "Missing Wine server: $out/bin/wineserver" >&2
          exit 1
        fi
        ...
        create_hosted_runtime_layout() {
          ...
          host_loader="$(find "$out/lib/wine" -path '*/wine' -type f | head -n 1)"
          if [ -z "$host_loader" ]; then
            echo "Missing Wine Unix loader under: $out/lib/wine" >&2
            exit 1
          fi

          rm -rf "$hosted_dir" "$original_bin"
          mv "$out/bin" "$original_bin"
          mkdir -p "$hosted_dir"
          find "$original_bin" -mindepth 1 -maxdepth 1 -exec mv '{}' "$hosted_dir/" \;
          rmdir "$original_bin"

          wine_stub="$hosted_dir/wine"
          if [ ! -f "$wine_stub" ]; then
            echo "Hosted runtime is missing the Wine loader stub." >&2
            exit 1
          fi

          cp -f "$wine_stub" "$hosted_dir/wineloader"
          chmod u+w,ugo+x "$hosted_dir/wineloader"
          if [ ! -e "$hosted_dir/wineserver" ]; then
            echo "Hosted runtime is missing wineserver." >&2
            exit 1
          fi
    ...
    External GPTK/D3DMetal import: ${
      if supportsExternalGptkD3DMetal then
        "supported when supplied by the user"
      else
